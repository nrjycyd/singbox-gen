...
| **规则编辑** | 可视化维护分流规则：规则集增删改；DNS/Route 规则**拖动排序**（或 ↑↓）、编辑、复制、删除；按动作**向导式添加**规则 |

规则编辑保存时只替换 YAML 中的 `rule_sets` / `dns_rules` / `route_rules` 三个节点，
其余内容与注释原样保留（服务端用 `yaml.Node` 定点改写、2 空格缩进）；保存后左侧 YAML 自动刷新。
